class Api::V1::TransactionsController < Api::V1::ApplicationController
    ...
    def list
        ...
        if query[:keywords].present?
            items = items.where("item_name ILIKE ? or invoice_number ILIKE ? OR provider ILIKE ? OR item_type ILIKE ? ","%#{query[:keywords]}%","%#{query[:keywords]}%","%#{query[:keywords]}%","%#{query[:keywords]}%")
        end
